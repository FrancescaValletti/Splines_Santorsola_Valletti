...
#aumentiamo il numero di controlpoints per aumentare il numero di input del calcolo parallelo

@everywhere function plarTensorProdSurface{F<:Function, T<:Real}(args::Array{F, 1}, 
                            controlpoints::Array{Array{Array{T, 1}, 1}, 1})
    ubasis , vbasis = args
    function map_fn(point::Array{Float64, 1})
        u, v = point
        U = ubasis([u])#numero polinomi prima base
        V = vbasis([v])
	...
	retp=SharedArray(ret)
        for i in 1:dim_u
            for j in 1:dim_v
                #for M in 1:target_dim
                        @sync @parallel for M in 1:target_dim 
                        retp[M] += U[i]*V[j]*controlpoints[i][j][M]
                    end
                #end
            end
        end
...
@everywhere  function plarBilinearSurface(controlpoints)
   basis = larBernsteinBasis(sel(1), 1)
   return plarTensorProdSurface([basis, basis], controlpoints)
end

 
@everywhere function plarBiquadraticSurface(controlpoints)
   basis1 = larBernsteinBasis(sel(1), 2)
   basis2 = larBernsteinBasis(sel(1), 2)
   return plarTensorProdSurface([basis1,basis2], controlpoints)
end

@everywhere function plarBicubicSurface(controlpoints)
   basis1 = larBernsteinBasis(sel(1), 3)
   basis2 = larBernsteinBasis(sel(1), 3)
   return plarTensorProdSurface([basis1, basis2], controlpoints)
end

@everywhere function plarBezier{T<:Real}(u::Function, controldata::Array{Array{T,1},1})
	...
	oup=SharedArray(out)
        @sync @parallel for i in 0:n
             weight = binomial(n,i)*(1-t)^(n-i)*t^i
             for k in 1:dim
                oup[k] += weight*controldata[i+1][k]
            end
        end
        return oup
    end
...
function plarBezierCurve(controlpoints)
   return plarBezier(sel(1), controlpoints)
end


#anche in questo caso 

function plarCoonsPatch{T<:Function}(args::Array{T, 1})
...
function pbsplineBasis{T<:Real}(degree::Int64, knots::Array{T,1}, ncontrols::Int64)
    n = ncontrols - 1
    m = length(knots) - 1
    k = degree + 1
    ...
    function map_fn{T<:Real}(point::Array{T, 1})
        t = point[1]
        s=[@spawn deBoor(i,k,t) for i in 1:n+1]
	sp=zeros(Float64, length(s))
	for j in 1:length(s)
	sp[j]=fetch(s[j])
	end
	return sp 
...
function pt_bspline{T<:Real, F<:Function}(u::Function, degree::Int64, knots::Array{T,1}, 
                                    points_fn::Array{F, 1})
    ...
    function deBoor(i::Int64, k::Int64, t::Float64)
        ...
        num1, div1 = t-knots[i], knots[i+k-1]-knots[i]  
        if div1 != 0 
            retp+=(num1/div1) * deBoor(i,k-1,t)
            # elif num1!=0: ret+=deBoor(i,k-1,t)
        end
        
        num2, div2 = knots[i+k]-t, knots[i+k]-knots[i+1]
        if div2!=0  
            retp+=(num2/div2) * deBoor(i+1,k-1,t)
	...
	retp=SharedArray(ret)
        @sync @parallel for i in 1:n+1
            coeff = deBoor(i,k,t) 
            for m in 1:target_dim
                retp[m] += points_fn[i](point)[m] * coeff
            end
        end
        return retp
    end
...
function pt_nurbs{T<:Real, F<:Function}(u::Function, degree::Int64, knots::Array{T,1}, 
                                       points::Array{F, 1})
    bspline = pt_bspline(u, degree, knots, points)
